...
/** ClassLoader that is used to reflectively invoke presentation compiler APIs.
  ...
  * the compiler while Metals only runs in a single Scala version. In order to
  * communicate between Metals and the reflectively loaded compiler, this
  * classloader shares a subset of Java classes that appear in method signatures
  * of the `PresentationCompiler` class.
  */
